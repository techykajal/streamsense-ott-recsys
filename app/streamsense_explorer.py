...
# ---- Tab 3 ----
with tab3:
    st.subheader("Per-model offline evaluation (real numbers, real dataset)")
    mp = "metrics/eval_metrics.json"
    if os.path.exists(mp):
        m = json.load(open(mp)); cols = st.columns(len(m) or 1)
        for col, (name, d) in zip(cols, m.items()):
            with col:
                st.markdown(f"**{name}**")
                for k, v in d.items():
                    st.metric(k, f"{v:.4f}" if isinstance(v, (int, float)) else v)
        st.caption("Generated by notebooks/StreamSense_Evaluation.ipynb")
    else:
